...
* @author: cts
* @createTime: 2017年12月18日 下午5:11:03
* @history:
* @version: v1.0
 */
@Controller
...
public class RefundController extends BaseAPIController {
    ...
    public Map<String, Object> process(HttpServletRequest request) {
        ...
        try {
            ...
            if (StringUtil.isBlank(req)) {
                ...
                resultMap.put("returnMsg", "退款请求参数为空");
                return resultMap;
            }
            // step3 验证参数合法性
            ...
            // step5 执行业务逻辑操作
            OuterRefundRequest refundRequest = JSONUtil.parseObject(req, OuterRefundRequest.class);
            OuterRefundResponse outerRefundResponse = doPayBiz.doRefund(refundRequest);
            resultMap = ConvertUtil.objectToMap(outerRefundResponse, null);
	...
	protected boolean verifyParameter(String req, Map resultMap) {
        ...
        try {
            ...
            logger.info("退款参数转换异常");
            resultMap.put("returnCode", "FAIL");
            resultMap.put("returnMsg", "退款参数转换异常");
            return false;
        }
